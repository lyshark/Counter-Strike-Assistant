...
DWORD engineModuleHandle = 0;
DWORD MatrixBaseAddress = 0;
...
int main(int argc, char *argv[])
{
  ...
  engineModuleHandle = GetProcessModuleHandle(ProcessPID, L"engine.dll");
  MatrixBaseAddress = engineModuleHandle + HexToDec("0x596EF0");

  printf("ModuleHandle: %d --> Matrix base address: 0x%x \n\n", engineModuleHandle, MatrixBaseAddress);

  FLOAT ViewWorld[4][4] = { 0 };
  while (1)
  {
    // Read in matrix [vertical matrix]
    // Matrix ViewWorld[4][4] => 4*4=16 => 16*word = 64
    ReadProcessMemory((HANDLE)ProcessHandle, (PVOID)MatrixBaseAddress, ViewWorld, 64, 0);
